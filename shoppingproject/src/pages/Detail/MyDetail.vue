    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <MyZoom :skuImageList="skuImageList"></MyZoom>
          <!-- 小图列表 -->
          <MyImageList :skuImageList="skuImageList"></MyImageList>
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前我们的路由跳转：从a路由跳转到b路由，这里在加入购物车，进行路由跳转，发请求把你购买的产品的信息通过请求的形式通知服务器，服务器进行相应的储存 -->
...
<script>
import MyImageList from "./ImageList/MyImageList.vue";
import MyZoom from "./Zoom/MyZoom.vue";
//引入小仓库辅助函数
...
export default {
  ...
  mounted() {
    //详情页---获取action产品详情信息(动态数据)
    this.$store.dispatch("getGoodInfo", this.$route.params.skuId);
  },
  computed: {
    //详情页---动态数据渲染---获取三级联动路由导航数据---产品展示信息---产品售卖属性
  ...
  methods: {
    ...
    async addShopcar() {
       ...
       * this.$store.dispatch("addOrUpdateShopCart", {
        ...
        上面这个代码说明了：调用仓库中的addOrUpdateShopCart
        这个方法加上async，返回一定是一个Promise
        要么成功，要么失败
       */
      try {
        ...
        //产品信息的数据[比较复杂：skuInfo]，通过会话储存（不持久化，会话结束数据再消失）
        //本地存储|会话存储，一般存储的是字符串
